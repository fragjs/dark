...
function Module(builder, modulePath, path, extension, content) {
...
Module.prototype = {
	...
	addRef: function () {
		...
		// module.refs should be added either.
		for (var ref in module.refs) {
			var to = this.refs[ref] || (this.refs[ref] = []),
				from = module.refs[ref];
		...
		if (this.refs[module.extension]) {
			this.refs[module.extension].push(module);
		} else {
			this.refs[module.extension] = [module];
		}
	},
	...
	include: function (modulePath) {
		var module = this.builder.getModule(modulePath);
...
Builder.prototype = {
	...
	build: function () {
		...
		// Do parse file.
		//this.include(configs.path);

		// Get the current module.
		var module = this.getModule(this.path);
	 ...
	 */
	include: function (modulePath) {
		var url = this.toUrl(modulePath),
			extension = this.getExtension(url);

		// Do not load twice.
		if (!this.isLoaded(url, extension)) {

			// Mark as loaded.
			include.loaded.push(url);

			// Save included modulePath.
			include.included.push(modulePath);

			var sourceCode = IO.readFile(url, this.fromEncoding);

			// No sourceCode or file not exists, nothing parsed.
			if (sourceCode) {

				var refList = this.refs[extension] || (this.refs[extension] = []);
	...
	getModule: function (modulePath) {
		...
		var path = this.toPath(modulePath);
		...
		var module = modules[path] = new Module(this, modulePath, path, this.getExtension(path), IO.readFile(path, this.fromEncoding));

		// No sourceCode or file not exists, nothing parsed.
		if (module.content) {

			// Do load.
			this.fileExtensions[module.extension](module);
	...
	getExtension: function (url) {
		...
		return match && (match[0] in this.fileExtensions) ? match[0] : ".html";
	},
	...
	isLoaded: function (url, extension) {
		return this.loaded.indexOf(url) >= 0;
	},
	...
	fileExtensions: {
		'.js': function (module) {
			...
			//if (builder.addModulePath !== false) {
			//	output.push(
			//		builder.lineBreak,
			//		"/*********************************************************",
			//		builder.lineBreak,
			//		" * ",
			//		Path.relative(builder.basePath, module.path).replace(/\\/g, "/"),
			//		builder.lineBreak,
			//		" ********************************************************/",
			//		builder.lineBreak
			//	);
			...
			try {
				...
				// Output the ast.
				output = printAstToString(ast, context);

			} catch (e) {

				this.error("Parse File Error: " + e.toString() + ' (' + path + ')');

				output = module.content;
			}

			module.outputs.js = output;
		...
		'.css': function (path, sourceCode, context) {
			...
			if (context.addModulePath !== false) {
				output.push(
					...
					context.lineBreak,
					" * ",
					Path.relative(context.basePath, path).replace(/\\/g, "/"),
					context.lineBreak,
					" ********************************************************/",
					context.lineBreak
				);
			...
			output.push(sourceCode);

			context.print('css', output, path);
	...
	jsPrinters: [
		...
		function (outputs, context) {
			...
			if (outputs.css && outputs.css.length) {
				...
				// All css code should be wrapped with <style>.
				r.push('document.write(', toJsString('<style type="text/css">' + outputs.css.join('') + '</style>'), ');', context.lineBreak);
			}

			if (outputs.html && outputs.html.length) {

				// All html code should be wrapped with document.write.
				r.push('document.write(', toJsString(outputs.html.join('')), ');', context.lineBreak);
			}
	...
	exclude: function (modulePath) {
		...
		context.excludes.push(modulePath);
	},
...
exports.build = function (configs) {
	...
	var context = Helpers.extend({
		...
		print: function (type, content, path){
			...
			outputs.push.apply(outputs, content);
		},
